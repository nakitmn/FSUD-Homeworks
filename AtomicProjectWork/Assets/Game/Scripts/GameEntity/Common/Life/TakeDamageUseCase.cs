...
namespace SampleGame
{
    ...
    {
        public static bool TakeDamage(in IGameEntity target, in int damage, IGameEntity source)
        {
        ...
        public static void TakePointDamage(Vector3 position, float radius, int damage, IGameEntity owner)
        {
            var colliders = RaycastUseCase.ScanTargets(position, radius);
            foreach (var collider in colliders)
            {
                if (collider.TryGetComponent(out IGameEntity entity))
                {
        ...
        public static int GetExtraDamage(int damage, IGameEntity owner)
        {
